...
GEN_BEHAVIOR_DATA_PATH = @(fname) fullfile(pwd, 'FigureCode', 'fig4', 'BehaviorData', fname);
...
print(gcf,'-dpdf',GEN_SAVE_PATH('fig4-fish-behavior.pdf'));
...
axesPosition(1:2) = [0.3, 0.3];
set(gca, 'Position', axesPosition);
print(gcf,'-dpdf',GEN_SAVE_PATH('fig4-fish-simulation.pdf'));
...
axesPosition(1:2) = [0.3, 0.3];
set(gca, 'Position', axesPosition);
print(gcf,'-dpdf',GEN_SAVE_PATH('fig4-mole-behavior.pdf'));
...
axesPosition(1:2) = [0.3, 0.3];
set(gca, 'Position', axesPosition);
print(gcf,'-dpdf',GEN_SAVE_PATH('fig4-mole-simulation.pdf'));
...
EH_hSNR_files = dir(GEN_DATA_PATH('fig4-ErgodicHarvest-Cockroach-StrongSignal-RandSeed-*.mat'));
IT_lSNR_files = dir(GEN_DATA_PATH('fig4-Infotaxis-Cockroach-WeakSignal-RandSeed-*.mat'));
IT_hSNR_files = dir(GEN_DATA_PATH('fig4-Infotaxis-Cockroach-StrongSignal-RandSeed-*.mat'));
...
for i = 1:length(EH_lSNR_files)
    ...
    % Relative exploration effort - Angular distance traveled
    EH_hSNR.moleDist = cumDist(EH_hSNR.sTrajList);
    EH_lSNR.moleDist = cumDist(EH_lSNR.sTrajList);
    IT_hSNR.moleDist = cumDist(IT_hSNR.sTrajList);
    IT_lSNR.moleDist = cumDist(IT_lSNR.sTrajList);
...
hBoxPlot = notBoxPlot([re4', re12'], ...
    [ones(1,length(re4)), 2*ones(1,length(re12))]);
opt = [];
opt.BoxDim = [10,5.4]*0.82;
opt.YLabel = 'Relative Exploration'; % ylabel
opt.YLim = [0, 10];
opt.YTick = [1, 2, 4, 6, 8, 10];
opt.ShowBox = 'off';
opt.XMinorTick = 'off';
opt.YMinorTick = 'off';
opt.FontName = 'Helvetica';
...
hLine(1).Marker = '.';
hLine(3).Marker = '.';
hLine(1).MarkerEdgeColor = ones(1,3)*0.0;
hLine(3).MarkerEdgeColor = ones(1,3)*0.0;
hLine(1).MarkerSize = 30;
hLine(3).MarkerSize = 30;
hLine(2).LineWidth = 5;
hLine(4).LineWidth = 5;
...
set(gca,'XTickLabel',{...
    '\color[rgb]{0.1961,0.7059,0.2902}Strong Signal',...
    '\color[rgb]{0.6353,0,0}Weak Signal'})
set(gca,'YTickLabel',{'1x', '2x', '4x', '6x', '8x', '10x'})
set(gca, 'units', 'normalized');
axesPosition = get(gca, 'Position');
axesPosition(1:2) = [0.3, 0.3];
set(gca, 'Position', axesPosition);
print(gcf,'-dpdf',GEN_SAVE_PATH('fig4-cockroach-behavior.pdf'));
