...
plot(freqTicks, refugeMag, ...
        'LineWidth', 2, 'Color', barColors(1, :));
plot(freqTicks, fishMagHSNR(2, :), ...
        'LineWidth', 2, 'Color', cmap(1, :));
plot(freqTicks, fishMagLSNR(1, :), ...
...
plot(freqTicks, trajMag, ...
        'LineWidth', 2, 'Color', [75, 111, 182]/255);
plot(freqTicks, sensorMagHSNR(3, :), ...
        'LineWidth', 2, 'Color', cmap(1, :));
plot(freqTicks, sensorMagLSNR(3, :), ...
    'LineWidth', 2, 'Color', cmap(2, :));
xlabel('Frequency');
ylabel('Fourier Magnitude');
opt = [];
...
patch(...
    ...
    'FaceColor', 'k', 'FaceAlpha', 0.1, ...
    'EdgeAlpha', 0)
print(gcf,'-dpdf',GEN_SAVE_PATH('fig5-electric-fish.pdf'));
...
%%% Spectrum (Lateral) %%%
axes; hold on;
plot(fftTicks, MoleFreqMagRawX_lSNR(:, 4), '-', ...
    'LineWidth', 2, 'Color', [0.5, 0, 0, 0.9]);
plot(fftTicks, MoleFreqMagRawX_hSNR(:, 4), '-', ...
...
%%% Spectrum %%%
axes; hold on;
plot(fftTicks, MoleEIHMag_lSNR(:, 1), '-', ...
    'LineWidth', 2, 'Color', [0.5, 0, 0, 0.9]);
plot(fftTicks, MoleEIHMag_hSNR(:, 1), '-', ...
    'LineWidth', 2, 'Color', [0, 0.5, 0, 0.9]);
set(gca, 'YTick', []);
...
print(gcf,'-dpdf',GEN_SAVE_PATH('fig5-mole.pdf'));
...
axes; hold on;
plot(fftTicks, pltMagY_lSNR(:, 7), '-', ...
    'LineWidth', 2, 'Color', [0.5, 0, 0, 0.9]);
plot(fftTicks, pltMagY_hSNR(:, 7), '-', ...
    'LineWidth', 2, 'Color', [0, 0.5, 0, 0.9]);
set(gca, 'YTick', []);
set(gca, 'XTick', [0, 2]);
set(gca, 'FontName', 'Helvetica');
...
%%% Spectrum %%%
axes; hold on;
plot(Cockroach_EIH_lSNR.fftTicks, Cockroach_EIH_lSNR.fftMag, '-', ...
    'LineWidth', 2, 'Color', [0.5, 0, 0, 0.9]);
plot(Cockroach_EIH_hSNR.fftTicks, Cockroach_EIH_hSNR.fftMag, '-', ...
    'LineWidth', 2, 'Color', [0, 0.5, 0, 0.9]);
set(gca, 'YTick', []);
set(gca, 'XTick', [0, 1]);
set(gca, 'FontName', 'Helvetica');
...
p = kruskalwallis(magListEIH, magLabelEIH, 'off');
print(gcf,'-dpdf',GEN_SAVE_PATH('fig5-cockroach.pdf'));
...
for i = 1:length(hsnrFiles)
    ...
    if MATCH_IDX
        ...
        hsnrBodeGain(i, :) = sFFT(oLocs) ./ oFFT(oLocs);
    end
...
for i = 1:length(lsnrFiles)
    ...
    if MATCH_IDX
        ...
        lsnrBodeGain(i, :) = sFFT(oLocs) ./ oFFT(oLocs);
    end
    fprintf('file %s processed\n', lsnrFiles(i).name);
end
...
axes; hold on;
notBoxPlot(medGainData, medGainLabel, 'jitter', 0.05, 'alpha', 0.5);
xlim([0.5, 2.5]);
ylim([1, 2.1]);
set(gca, 'YTick', [1, 2]);
set(gca, 'XTick', [1, 2]);
...
set(gca, 'Position', axesPosition);
title('Behavior', 'FontSize', 16)
print(gcf,'-dpdf',GEN_SAVE_PATH('fig5-moth.pdf'));
% Test statistics (non-parametric ANOVA)
