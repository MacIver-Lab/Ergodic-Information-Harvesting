...
if ~legacyCall
    ...
    params.addParameter('plotRawData', false, @(x) islogical(x));
...
%        h(k).sdPtch=patchMaker(SD(k),[0.6,0.6,1]);
     ...
     if plotRawData
         h(k).data=plot(thisX+J, thisY, 'o', 'color', C,...
             'markerfacecolor', C+(1-C)*0.65, 'markersize', 4);
     end
     ...
     if strcmp(style,'patch') || strcmp(style,'sdline')
       ...
       h(k).mu=plot([X(k)-jitScale,X(k)+jitScale],[mu(k),mu(k)],['-',plotColor],...
            'linewidth',2);
       if markMedian
          statsOut(k).median = med(k);
          h(k).med=plot([X(k)-jitScale,X(k)+jitScale],[med(k),med(k)],[':',plotColor],...
                'linewidth',2);
       end
     end
 end
