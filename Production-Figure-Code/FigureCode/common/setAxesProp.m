function h = setAxesProp(opt, haxe)
...
if isfield(opt, 'BoxPos')
    BoxPos = [opt.BoxPos(1), opt.BoxPos(2), BoxDim(1), BoxDim(2)];
end
% BoxPos = [1, 1, BoxDim(1), BoxDim(2)];
...
IgnoreLines = 0;
if isfield(opt, 'IgnoreLines')
    IgnoreLines = opt.IgnoreLines;
end
...
if HoldLines == false
    for ii=1:N   
        ...
        if ~IgnoreLines && strcmp(hp(ii).Type,'line') && ~strcmp(hp(ii).UserData,'Ignore')
            set(hp(ii)          , ...
